...
object Resolution {
    ...
    fun getScreenSize(context: Context, outSize: Point?): Point {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            ...
            ret.x = displayContext.resources.displayMetrics.widthPixels
            ret.y = displayContext.resources.displayMetrics.heightPixels
        } else {
            val defaultDisplay = wm.defaultDisplay
            val displayMetrics = DisplayMetrics()
            defaultDisplay.getMetrics(displayMetrics)
            ret.x = displayMetrics.widthPixels
            ret.y = displayMetrics.heightPixels
        }
        return ret
    }
    ...
    fun getNavigationBarSize(context: Context): Point {
        ...
        return if (appUsableSize.y < appUsableSize.y) {
            Point(appUsableSize.x, appUsableSize.y - appUsableSize.y)
        } else Point()
